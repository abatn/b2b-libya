...
(function() {
    ...
    const _base = window.location.pathname.startsWith('/ar/') ? '/ar' : '';
    ...
    async function loadCategories() {
        try {
            const res = await fetch('/api/b2b/categories');
    ...
    function setupAutocomplete() {
        ...
        input.addEventListener('input', function() {
            ...
            acTimeout = setTimeout(async () => {
                try {
                    const res = await fetch('/api/b2b/products?search=' + encodeURIComponent(q) + '&limit=6');
    ...
    async function loadNotifications() {
        try {
            const res = await fetch('/api/b2b/messages/unread-count');
            if (res.ok) {
                const data = await res.json();
    ...
    function generateBreadcrumb() {
        ...
        const path = window.location.pathname.replace(/^\/ar\//, '/').replace(/\/$/, '') || '/';
