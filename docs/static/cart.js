...
const Cart = {
    ...
    async add(productId, quantity = 1, supplierId = null) {
        try {
            ...
            const res = await fetch(`${window.API_BASE}/api/cart/items`, {
                method: 'POST',
    ...
    async updateQuantity(itemId, quantity) {
        try {
            await fetch(`${window.API_BASE}/api/cart/items/${itemId}?quantity=${quantity}`, { method: 'PUT' });
            await this.updateBadge();
        } catch (e) {
    ...
    async remove(itemId) {
        try {
            await fetch(`${window.API_BASE}/api/cart/items/${itemId}`, { method: 'DELETE' });
            await this.updateBadge();
        } catch (e) {
    ...
    async clear() {
        try {
            await fetch(`${window.API_BASE}/api/cart`, { method: 'DELETE' });
            await this.updateBadge();
        } catch (e) {
